    ...
    const fetchPrivacy = async () => {
      try {
        const response = await fetch(
          "https://api.stayro.com/ar/privacy_policy/api/privacy-policy-items/"
        );
        if (!response.ok) {
      ...
      <div className="flex justify-between w-[1400px] h-[3000px] mt-10">
        <div className="pic w-[1000px] h-[2900px] flex flex-col mt-20 mx-40">
          <div className="pic flex flex-col items-center">
            <h1 className="font-bold text-[44px] mt-4"> سياسات الخصوصية</h1>
          </div>
          <div className="content mt-10 flex flex-col items-end">
            <div className="head flex gap-2">
              <p className="font-bold text-[18px]"> المقدمة </p>
              <Image
                src={Img}
                alt="Streo"
            ...
            <div className="mt-6">
              {PrivacyData && PrivacyData.length > 0 ? (
                ...
                <p> غير متوفرة في الوقت الحالي</p>
              )}
            </div>
          </div>
        </div>
